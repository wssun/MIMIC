# train clean encoder from scratch or fine-tune encoders
...
if __name__ == '__main__':
    ...
    parser.add_argument('--suffix_encoder', type=str, default='', help='the path to pretrained encoder')
    ...
    # note that the memory data and test_data_clean are only used to monitor the pre-training of the image encoder
    ...
    # Intialize the model
    model = get_encoder_architecture(args).cuda(device=args.device)
    if args.suffix_encoder != '':
        checkpoint = torch.load(args.suffix_encoder, map_location=args.device)
