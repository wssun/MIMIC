...
def train(backdoored_encoder, clean_encoder, data_loader, train_optimizer, args):
    ...
    for img_clean, img_backdoor_list, reference_list, reference_aug_list in train_bar:
        ...
        for i in range(len(feature_reference_list)):
            ...
            loss_1_list.append(
                - torch.sum(feature_reference_aug_list[i] * clean_feature_reference_list[i], dim=-1).mean())
        ...
        train_bar.set_description(
            'Train Epoch: [{}/{}], lr: {:.6f}, Loss: {:.6f}, Loss0: {:.6f}, Loss1: {:.6f},  Loss2: {:.6f}'
                .format(epoch,
                        args.epochs,
...
def load_downstreamTask():
    # load downstream task
    # just for test when training badencoder
    from datasets.cifar10_dataset import CIFAR10Mem, test_transform_cifar10
    from datasets.stl10_dataset import test_transform_stl10
    from datasets.backdoor_dataset import BadEncoderTestBackdoor
    downstream = 'data/BadEncoder/' + args.downstreamTask
    test_transform = None
    if args.encoder_usage_info == 'cifar10':
        ...
        NotImplementedError()
    if args.downstreamTask == 'cifar10':
        from datasets.cifar10_dataset import classes
    elif args.downstreamTask == 'stl10':
        from datasets.stl10_dataset import classes
    elif args.downstreamTask == 'svhn':
        from datasets.svhn_dataset import classes
    elif args.downstreamTask == 'gtsrb':
        from datasets.gtsrb_dataset import classes
    else:
        classes = None
        NotImplementedError()
    _memory_data = CIFAR10Mem(numpy_file=downstream + '/train.npz', class_type=classes,
                              transform=test_transform)
    _test_data_backdoor = BadEncoderTestBackdoor(numpy_file=downstream + '/test.npz', trigger_file=args.trigger_file,
                                                 reference_label=args.reference_label,
                                                 transform=test_transform)
    _test_data_clean = CIFAR10Mem(numpy_file=args.data_dir + '/test.npz', class_type=classes,
                                  transform=test_transform)
    print('test backdoor, test clean: ', len(_test_data_backdoor), len(_memory_data))
    # import sys
    # sys.exit(-1)
    return _memory_data, _test_data_clean, _test_data_backdoor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretraining_dataset', default='cifar10', type=str, help='pretraining dataset')
    parser.add_argument('--shadow_dataset', default='cifar10', type=str, help='shadow dataset')
    parser.add_argument('--downstreamTask', default='cifar10', type=str, help='just for test our encoder')
    ...
    parser.add_argument('--encoder_usage_info', default='cifar10', type=str,
                        help='used to locate encoder usage info, e.g.,'
                             ' encoder architecture and input normalization parameter')
    ...
    args.data_dir = f'./data/BadEncoder/{args.shadow_dataset.split("_")[0]}/'
    args.knn_k = 200
    args.knn_t = 0.5
    args.reference_label = np.load(args.reference_file)['y'][0]
    print('reference_label: ', args.reference_label)
    print(args)
    if not os.path.exists(args.results_dir):
        os.mkdir(args.results_dir)

    # Create the Pytorch Datasets, and create the data loader for the training set memory_data, test_data_clean,
    # and test_data_backdoor are used to monitor the finetuning process. They are not reqruied by our BadEncoder
    shadow_data, _, _, _ = get_shadow_dataset(args)
    memory_data, test_data_clean, test_data_backdoor = load_downstreamTask()
    train_loader = DataLoader(shadow_data, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True,
                              drop_last=True)
    ...
    if args.encoder_usage_info == 'cifar10' or args.encoder_usage_info == 'stl10':
        # note that the following three dataloaders are used
        # to monitor the finetune of the pre-trained encoder,
        # they are not required by our BadEncoder. They can be ignored if you do not need to monitor the finetune of the pre-trained encoder
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.encoder_usage_info == 'cifar10' or args.encoder_usage_info == 'stl10':
            ...
            # the test code is used to monitor the finetune of the pre-trained encoder,
            # it is not required by our BadEncoder.
            # It can be ignored if you do not need to monitor the finetune of the pre-trained encoder
