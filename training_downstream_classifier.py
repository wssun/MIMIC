...
if __name__ == '__main__':
    ...
    parser.add_argument('--encoder_usage_info', default='', type=str,
                        help='used to locate encoder usage info,'
                             ' e.g., encoder architecture and input normalization parameter')
    ...
    parser.add_argument('--nn_epochs', default=1000, type=int)
    ...
    if args.encoder != '':
        ...
        checkpoint = torch.load(args.encoder, map_location='cuda:0')
    ...
    for epoch in range(1, args.nn_epochs + 1):
        ...
        if 'clean' in args.encoder:
            acc = net_test(net, nn_test_loader, epoch, criterion, 'Clean Accuracy (CA)')
            asr = net_test(net, nn_backdoor_loader, epoch, criterion, 'Attack Success Rate-Baseline (ASR-B)')
        else:
    ...
    # pd.DataFrame({'acc':acc,'asr':asr}).to_csv('results/'+str(args.encoder).replace('/','').replace('.','')+'.csv')
